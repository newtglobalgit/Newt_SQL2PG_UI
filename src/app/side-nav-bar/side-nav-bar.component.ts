...
export class SideNavBarComponent implements OnInit {
  ...
  async getDMAPImageDetails() {
    ...
    if (
      this.appContainerDown === false &&
      this.appNodeNotConfigured === false
    ) {
      let dbRes: any = [];
      let appRes: any = [];
      try {
  ...
  masterBackup() {
    ...
    modalRef.result.then((result) => {
      if (result == 'ok') {
        const appBackupRequired = modalRef?.componentInstance?.data
          ?.showRadioButtons
  ...
  private downloadKit(platform: string, kitType: string): void {
    ...
    if (kitType === 'data_migration') {
      ...
      downloadObservable =
        this.databaseListService.schemaValidationDMKit(platform);
    } else if (kitType === 'data_validation') {
      downloadObservable =
        this.databaseListService.dataValidationDMKit(platform);
    }
