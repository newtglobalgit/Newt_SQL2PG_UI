...
export class DmapMultipleSchemaDeleteComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    sessionStorage['assessmentSelectedDatabase'] = undefined; //commenting for NDMAP-4282 ticket
    this.spinner.hide();
    this.ngOnChanges();
    for (let k in this.schemasToDelete){
      if (this.schemasToDelete[k].runId == this.data.selectedSchema.runId){
        this.schemasToDelete[k].isRowChecked = !this.schemasToDelete[k].isRowChecked;
      }
    }
  ...
  setAllCheckBox(){
    ...
    if(_schemaList.length == this.schemasToDelete.length && this.schemasToDelete.length > 0){
  ...
  checkAll(action){
    ...
    for(let i in this.schemasToDelete){
      if(action == 'add' && this.checkedSchemas.indexOf(this.schemasToDelete[i].runId) == -1){
        this.checkedSchemas.push(this.schemasToDelete[i].runId);
      }else if(action == 'remove'){
        this.checkedSchemas.splice(this.checkedSchemas.indexOf(this.schemasToDelete[i].runId), 1)
      }
    }
  ...
  submit(){
    ...
    for (let i in this.schemasToDelete){
      ...
      for (let j in this.checkedSchemas){
        if(this.schemasToDelete[i].runId == this.checkedSchemas[j]){
          ...
          reqObj.push({"runId":this.schemasToDelete[i].runId,"assigned_vm":this.schemasToDelete[i].assigned_vm})
        }
      }
    }
    ...
    const modalRef = this.modalService.open(NgbdConfirmationModal);
      modalRef.componentInstance.data = {msg : 'Are you sure you want to delete selected schemas?', title : 'Confirmation',okButtonLabel : 'Yes',cancelButtonLabel:'No',label:'moveToCompletion'};
      modalRef.result.then((result) => {
        if ( result === 'ok') {

          this.spinner.show();
          let selecteDatabase_ = sessionStorage.getItem('assessmentSelectedDatabase');
          let selecteDatabase;
          ...
          this.databaseListService.mutiple_schema_delete({"RUN_IDS":runIds}).then((data) => {
